-- 1 AS query - Niiden ohjaajien nimet, jotka käyttävät appia. - Show instructors
 
SELECT fname AS 'Etunimi', lname AS 'Sukunimi' FROM instructors;

-- 2 OR query - Mitä tunteja on päivinä 22.03.2021 tai 25.03.2021? - Classes on dates 3/22/21 or 3/25/21
...
-- 3 AND query - Onko tanssia 23.3.2021 ja mihin aikaan?  Is there dance class 3/23/2021 and what time?

SELECT scheduled, startTime, endTime FROM Classes WHERE scheduled = '20210323' AND typeID = '4';

-- 4 Queries containing aggregate function - Luokittele ohjaajat sen mukaan, kenellä on eniten bookattuja tunteja. - Show list of instructors and
-- arrange by number of classes for each
...
-- 5 Paljonko maksaa kallein ohjaus? - What class is the most expensive?

SELECT MAX(classes.price) AS 'Kallein ohjaus' FROM Classes;
 
-- 6 GROUP BY query - Paljonko eri ohjauksia on bookattu? - How many reservations for different classes?
...
-- 7 HAVING query - Näytä ohjaukset, jotka maksavat alle 10 €. - Show classes below 10 €.
...
-- 8 LEFT JOIN query - Näytä ohjaajat, joilla ei ole bookattuja ohjauksia - Show instructors with no with no reservations.
...
-- 10 Kuinka monta osallistujaa on kullakin ohjauksella? - How many participants for each class?
